...
def main():
    try:
        ...
        s3_output_path = "s3://rawdatacontactar/fsd010_destinos_credito"
        ...
        SELECT DISTINCT 
            ...
            GETDATE() as Fecha
        FROM FSD010 FS
        INNER JOIN xwf700 XW ON FS.Aocta = XW.XWfCuenta 
            AND FS.Aooper = XW.XWfOperacion 
            AND FS.Aocta = XW.XWFCta1
        INNER JOIN SNG001 SN ON XW.XWFPRCINS = SN.SNG001Inst 
            AND XW.XWfCuenta = SN.SNG001cta
        INNER JOIN SNG014 SG14 ON SN.SNG014COD = SG14.SNG014COD
        """
        
        source_data = read_using_jdbc(connection_name, sql_query)
        clear_s3_path(s3_output_path)
        write_to_s3_parquet(source_data, s3_output_path, "write_fsd010_data")
