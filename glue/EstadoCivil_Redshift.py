...
def build_create_table_sql(spark_df, redshift_table):
    ...
    return f"""
        ...
        CREATE TABLE {redshift_table} (
                {columns_ddl},
                PRIMARY KEY (CodEstadoCivBT)
            )
            DISTKEY(CodEstadoCivBT)
            SORTKEY(CodEstadoCivBT);
    """
...
def main():
    try:
        ...
        redshift_table = "Dim.estadocivil"

        sql_query = """
        SELECT 
            CAST(Eccod AS INT) AS CodEstadoCivBT,
            Ecnom AS DesEstadoCivil,
            CAST(GETDATE() AS DATE) AS Fecha_carga_Bodega,
            CAST(GETDATE() AS DATE) AS Fecha_corte_info,
            CAST(1 AS VARCHAR(255)) AS Estado
        FROM FST009
        """
