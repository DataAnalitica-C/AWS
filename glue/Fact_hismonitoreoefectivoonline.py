...
def write_to_s3_csv(dynamic_frame):
    ...
    path = f"s3://archivos-compartidos-etls/hismonitoreoefectivoonline/fecha_corte={fecha_corte}/"
...
def main():
    try:
        ...
        redshift_table = "FACT.hismonitoreoefectivoonline"

        sql_query = """
		SELECT
			Itsuc AS COD_SUCURSAL,
			Modulo AS MODULO,
			Ittran AS TRANSACCION,
			Rubro AS RUBRO,
			SUM(Itimp1) AS IMPORTE,
			Itdbha AS NATURALEZA,
			GETDATE() AS FECHA_HORA
		FROM FSD016 WITH (NOLOCK)
		WHERE PgCod = 1
		AND Itdbha IN (1,2) -- diferencia entrada / salida
		AND Rubro IN (
			1105050015
			,1105050005
			,1105050013
