...
def create_view_in_redshift(creds):
    ...
    sql = "CREATE OR REPLACE VIEW Fact.vCreditos AS SELECT * FROM Fact.Historicocreditos where cast(Fechasistema as date)  = cast(getdate() as date) ;"
    ...
    stmt.close()
    conn.close()
    logger.info("Vista Fact.vCreditos creada/actualizada exitosamente.")
