...
def main():
    try:
        ...
        redshift_table = "Dim.Asesores"

        sql_query = """
        SELECT  C.SNGAS2Cod         AS CODIGO,
				...
				E.Valor             As EMAIL 
		FROM SNGAS2 C
		inner join FSE046 E             WITH (NOLOCK) ON        C.SNGAS2Usr = E.Ubuser
		LEFT JOIN DBO.FST746 T746       WITH (NOLOCK) ON        C.SNGAS2USR = T746.UBUSER
		LEFT JOIN DBO.FST046 F046       WITH (NOLOCK) ON        C.SNGAS2USR = F046.UBUSER 
																AND C.SNGAS2PGC = F046.PGCOD -- OFICINA ASESOR
		where Atributo = 'EMAIL'
