...
def main():
    try:
        ...
        redshift_table = "fact.hismonitoreosaldos"

        sql_query = """
            SELECT
        BCFech AS fecha_anterior,
        BCSuc AS bcsuc,
        BCRubr AS bcrubro,
        SUM(BCSdMN) * -1 AS saldo_anterior
    FROM FSH012 WITH (NOLOCK)
    WHERE BCEmp = 1 
      AND BCMda = 0
      --AND BCFech = CAST(DATEADD(DAY, -1, GETDATE()) AS DATE)
      AND BCFech = CONVERT(DATE, '2026-02-16')
      AND BCRubr IN (
            1105050015,
            1105050005,
            1105050013,
