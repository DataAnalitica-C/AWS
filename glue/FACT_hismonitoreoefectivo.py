...
def write_to_s3_csv(dynamic_frame):
    ...
    path = f"s3://archivos-compartidos-etls/hismonitoreoefectivo/fecha_corte={fecha_corte}/"
...
def main():
    try:
        ...
        redshift_table = "fact.hismonitoreoefectivo"

        sql_query = """
        SELECT
        Hsucor AS cod_sucursal,
        Hcmod AS modulo,
        Htran AS transaccion,
        Hrubro AS rubro,
        SUM(Hcimp1) AS importe,
        Hfvco AS fecha,
        Hcodmo AS naturaleza
        FROM FSH016
        WHERE PgCod = 1
        AND Hcodmo IN (1,2)
      AND Hcmod IN (51,50,94)
      AND Htran IN (179,175,155,150,110,610,156,158,159,163,167,168,210)
      AND Hrubro IN (
            1105050015, 1105050005, 1105050013,
            1105050003, 1105050002, 1105050001,
            1105050009, 1115050001, 1105050018
      )
      --AND Hfvco = CAST(DATEADD(DAY, -1, GETDATE()) AS DATE)
      AND Hfvco = '2026-02-16'
      AND Htpoasr <> 'A'
    GROUP BY 
        Hfvco,
        Hsucor,
