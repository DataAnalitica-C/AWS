...
def main():
    try:
        ...
        s3_output_path = "s3://rawdatacontactar/fst013_paises"
        
        sql_query = "SELECT Pais, Panom, GETDATE() AS Fecha1, GETDATE() AS Fecha2 FROM FST013"
        
        source_data = read_using_jdbc(connection_name, sql_query)
        clear_s3_path(s3_output_path)
        write_to_s3_parquet(source_data, s3_output_path, "write_fst013_paises")
