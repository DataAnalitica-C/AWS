...
def main():
    try:
        ...
        s3_output_path = "s3://rawdatacontactar/fbc206_ruralidad_DANE"
        
        sql_query = """
        SELECT  
            BC206Id2,
            CASE 
                WHEN BC206Nro2 = 1 THEN 'RURAL'
                ELSE 'URBANO'
            END AS RURALIDAD
        FROM FBC206 C206X 
        WHERE C206X.BC205EMP = 1 AND C206X.BC205COD = 331
        """
        
        source_data = read_using_jdbc(connection_name, sql_query)
        clear_s3_path(s3_output_path)
        write_to_s3_parquet(source_data, s3_output_path, "write_fbc206_ruralidad")
