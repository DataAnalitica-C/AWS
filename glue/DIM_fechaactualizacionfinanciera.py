...
def main():
    try:
        ...
        redshift_table = "Dim.Fechaactualizacionfinanciera"

        sql_query = """
		
		/* ÚLTIMA FECHA FINANCIERA POR CLIENTE */ 
		SELECT
			Documento,
			Fecha_financiera,
			DATEDIFF(DAY, Fecha_financiera, CAST(GETDATE() AS DATE)) AS Dias_a_hoy,
			CASE 
				WHEN DATEDIFF(DAY, Fecha_financiera, CAST(GETDATE() AS DATE)) > 365 THEN 'Desactualizado'
				ELSE 'Actualizado'
			END AS Estado_financiera
		FROM (
			SELECT
				DECO850Ndc AS Documento,            
				Cast(DECO850A21 as date) AS Fecha_financiera,
				-- Selección del más reciente por Documento, enviando NULLs al final
				ROW_NUMBER() OVER (
					PARTITION BY DECO850Ndc
					ORDER BY 
						CASE WHEN DECO850A21 IS NULL THEN 1 ELSE 0 END,
						DECO850A21 DESC
				) AS rn
			FROM DECO850
		) t
