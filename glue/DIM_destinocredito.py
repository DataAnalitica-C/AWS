...
def main():
    try:
        ...
        redshift_table = "Dim.Destinocredito"

        sql_query = """
        SELECT DISTINCT    FS.Aocta,
          FS.Aooper,
          SG14.SNG014Dsc,
          GETDATE() as Fecha
		FROM FSD010          AS FS  WITH(NOLOCK)
		    INNER JOIN xwf700  AS XW  WITH(NOLOCK)  ON  FS.Aocta    =  XW.XWfCuenta  AND
		                            FS.Aooper    =  XW.XWfOperacion AND
		                            FS.Aocta        =   XWFCta1
		    INNER JOIN SNG001  AS SN WITH(NOLOCK)    ON  XW.XWFPRCINS  =  SN.SNG001Inst  AND
		                            XW.XWfCuenta  =  SN.SNG001cta
		    INNER JOIN SNG014  AS SG14          ON  sn.SNG014COD    =  SG14.SNG014COD
