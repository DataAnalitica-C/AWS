...
def main():
    ...
    try:
        ...
        s3_output_path_FST750_ActividadesCIIU = "s3://rawdatacontactar/FST750_ActividadesCIIU"
        
        # Carga de Actividades CIIU
        sql_query_FST750_ActividadesCIIU = """
            SELECT
                b.ActCod2,
                b.ActNom2,
                a.ActCod1,
                ...
                a.Act0705,
                a.Act0706,
                GetDate() FechaSistema
            FROM FST750 as a WITH(NOLOCK)
            inner join  FST751 as b on a.ActCod2 = b.ActCod2
        """
        source_data = read_using_jdbc(
            connection_name=connection_name,
            custom_sql_query=sql_query_FST750_ActividadesCIIU
        )

        # Clear S3 path for overwrite behavior
        clear_s3_path(s3_path=s3_output_path_FST750_ActividadesCIIU)

        write_to_s3_parquet(
            dynamic_frame=source_data,
            s3_path=s3_output_path_FST750_ActividadesCIIU,
            transformation_ctx="write_parquet_to_s3_fst750_ActividadesCIIU"
        )
        logger.info("Carga FST750 Actividades CIIU ejecutada y guardada en S3 correctamente")
    except Exception as e:
