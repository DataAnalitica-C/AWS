...
def main():
    try:
        ...
        create_fact_sql = """
        --DROP TABLE IF EXISTS Fact.historicocdt;
        --CREATE TABLE Fact.historicocdt AS
        INSERT INTO Fact.historicocdt
        SELECT a.*
           ,COALESCE(ds1.RegionNom, ds2.RegionNom) AS Nombre_region
           ,COALESCE(ds1.ZonaNom, ds2.ZonaNom) AS Nombre_zonal
        FROM Fact.captacionesfindia a
        LEFT JOIN Dim.sucursal ds1
        ON a.Asesor = ds1.codsucursal
        LEFT JOIN Dim.sucursal ds2
        ON a.Codigo_sucursal = ds2.codsucursal   
        """
